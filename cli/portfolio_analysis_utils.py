...
def analyze_portfolio(execute_trades: bool, min_conviction: float, analysis_date: Optional[str]):
    """
    Analyze your entire portfolio and get rebalancing recommendations.
    
    This command will:
    1. Fetch all current positions from your brokerage
    2. Analyze each position using the agent framework
    3. Calculate portfolio-level metrics
    4. Generate BUY/SELL/HOLD recommendations
    5. (Optional) Execute high-conviction trades
    6. Provide strategic insights and future recommendations
    """
    console.print("\n")
    console.print(
        Panel(
    ...
    selections = _get_portfolio_analysis_selections(execute_trades, analysis_date)
...
def run_portfolio_analysis_from_selections(selections: dict) -> None:
    ...
    with console.status("[bold green]Analyzing portfolio positions..."):
        results = analyzer.analyze_portfolio(execute_trades=execute_trades, min_conviction=min_conviction)
...
def _get_portfolio_analysis_selections(execute_trades: bool, analysis_date: Optional[str]) -> dict:
    ...
    console.print(create_question_box(
        "Step 1: Analysts Team", "Select your LLM analyst agents"
    ))
    selected_analysts = select_analysts()
    
    # Get research depth
    console.print(create_question_box(
        "Step 2: Research Depth", "Select analysis depth"
    ))
    selected_research_depth = select_research_depth()
    
    # Get LLM provider
    console.print(create_question_box(
        "Step 3: LLM Provider", "Select which service to use"
    ))
    selected_llm_provider, backend_url = select_llm_provider()
    
    # Get thinking agents
    console.print(create_question_box(
        "Step 4: Thinking Agents", "Select your AI models"
    ))
...
def _display_portfolio_analysis_results(results: Dict[str, Any], executed: bool):
    """Display portfolio analysis results in formatted panels."""
